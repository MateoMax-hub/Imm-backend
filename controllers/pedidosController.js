...
exports.crearPedido = async (req, res) => {
    ...
    try {
        ...
        const actualizarSaldo = await Usuario.findOneAndUpdate(
            ...
            { new: true }
        ).select('_id balance nombre apellido');
        await pedido.save();
...
exports.actualizarPrimera = async (req, res) => {
    ...
    try {
        ...
        const primeraEtapaUpdate = await Pedido.findOneAndUpdate({ _id: idPedido }, primeraEtapa, {
            new: true,
        });
...
exports.actualizarTercera = async (req, res) => {
    ...
    try {
        ...
        const terceraEtapaUpdate = await Pedido.findOneAndUpdate({ _id: idPedido }, terceraEtapa, {
            new: true,
        });
        await transporter.sendMail({
            from: '<ImmEditLeGras12@gmail.com>',
...
exports.cancelarPedido = async (req, res) => {
    ...
    try {
        ...
        if (pedido.primeraEtapa.estado === 'pendiente') {
            const reembolso = await Usuario.findOneAndUpdate(
                { _id: req.usuario.id },
                { $inc: { balance: pedido.precio } },
                { new: true }
            );
            await transporter.sendMail({
                from: '<ImmEditLeGras12@gmail.com>',
...
exports.obtenerPedidos = async (req, res) => {
    try {
        ...
        const pedidos = await Pedido.find()
            .populate('pack', 'descripcion titulo')
            .populate('consumidor', 'nombre apellido email');
        res.send(pedidos);
    } catch (error) {
        console.log(error);
    }
