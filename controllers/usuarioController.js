...
exports.actualizarCartera = async (req, res) => {
    try {
        const { body, usuario } = req;
        if (body.balance) {
            const balanceActualizado = await Usuario.findOneAndUpdate(
                { _id: usuario.id },
...
exports.obtenerUsuariosFiltrado = async (req, res) => {
    ...
    const usersFiltrados = await users.filter(
        (u) =>
            u.nombre.toLowerCase().includes(req.body.filtro.toLowerCase()) ||
